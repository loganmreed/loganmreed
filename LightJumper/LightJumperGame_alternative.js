...
const gridSizeX = 74; // Horizontal grid size
const gridSizeY = 72; // Vertical grid size
const boxWidth = 70; // Box width
const boxHeight = 70; // Box height

const marginBottomPercentage = 0.07; // 7% bottom margin
const screenHeight = window.innerHeight;
const marginBottom = screenHeight * marginBottomPercentage;

const marginRightPercentage = 0.10; // 10% right margin
...
const rows = Math.floor(availableHeight / boxHeight); // Rows based on height
const columns = Math.floor(screenWidth / gridSizeX); // Columns based on width
...
function updateCharacterPosition() {
    ...
    // Prevent moving past the top
    if (nextPosition.y < 0) {
        nextPosition.y = 0;
        characterVelocity.y = 0;
    }

    // Prevent moving past the floor
...
// Adjust character speed based on window width
...
function checkCollisions() {
    ...
    let standingOnSomething = false; // Track standing status
    ...
    // Check collision with keytwo if needed
    if (Needkeytwo) {
    ...
    // Check collision with wall if needed
    if (NeedWall) {
        ...
        if (characterRight > wallBounds.left && characterLeft < wallBounds.right && characterBottom > wallBounds.top && characterTop < wallBounds.bottom) {
            ...
            // Vertical collision (standing on the wall)
    ...
    // Check collision with skyplatform if needed
    if (Needskyplatform) {
        ...
        if (characterRight > skyplatformBounds.left && characterLeft < skyplatformBounds.right && characterBottom > skyplatformBounds.top && characterTop < skyplatformBounds.bottom) {
            ...
            // Vertical collision (standing on the platform)
            ...
            // Prevent character from passing through the bottom of the sky platform
            if (characterTop < skyplatformBounds.bottom && characterBottom > skyplatformBounds.bottom) {
                ...
                // character.style.top = `${skyplatformBounds.bottom}px`; // Uncomment to place the character on top of the platform
                if (!isCollidingWithPlatformBottom) {
                    isCollidingWithPlatformBottom = true; // Set the flag to true if colliding with the bottom
                    //skyplatform.style.background = '#808080'; // Change platform color on collision
                }
            }
        }
    
        // Check if character is no longer colliding with the bottom of the platform
    ...
    // Check collision with the door to move to the next level
    ...
    // Handle climbability and horizontal collision for boxes
    boxes.forEach((box) => {
        const boxColumn = box.getAttribute('data-column');
...
document.body.addEventListener('click', function (event) {
    ...
    // Prevent placing a box in columns occupied by the character
    if (column >= leftColumn && column <= rightColumn) {
        const message = document.createElement('div');
        message.classList.add('message');
        //message.textContent = `Cannot place a box in column ${column}, as the character is there.`;
...
function NextLevel(levelConfig) {
    // Set key position based on percentages
    ...
    // Clear boxes and increment level
    clearBoxes();
}

// Clear all boxes and reset grid
function clearBoxes() {
    boxes.forEach(box => box.remove()); // Remove all boxes
    boxes.length = 0; // Clear boxes array
    occupied.fill(0); // Reset occupied grid

    console.log("Boxes cleared. Occupied state reset.");
}

const level1Config = {
    keyXPercentage: 0.5, // Key X position is 50% of the screen width
    keyYPercentage: 0.80, // Key Y position is 80% of the screen height
    characterXPercentage: 0.2, // Character X position is 20% of the screen width
    characterYPercentage: 0.70, // Character Y position is 70% of the screen height
    maxBoxesPerColumn: 0, // Max boxes per column for this level
    keyColor: '#ff6600',  // Key color
};
