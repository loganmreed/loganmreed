...
public:
    ...
    void insert(const string &key) {
        ...
        int index = baseIndex;
        int i = 0;
        int firstDeleted = -1;  // Track first deleted slot
        ...
        while (table[index] != EMPTY) {
            ...
            if (table[index] == DELETED && firstDeleted == -1) {
                firstDeleted = index;  // mark first deleted slot
            }

            i++;
            index = (baseIndex + i * i) % SIZE;
            if (i >= SIZE) break; // probed all slots
        }

        // If we found a deleted slot, use it, otherwise use the empty slot
        if (firstDeleted != -1) {
